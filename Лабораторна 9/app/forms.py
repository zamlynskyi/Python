...
class RegistrationForm(FlaskForm):
    ...
    confirm_password = PasswordField('Підтвердіть пароль',
                                     validators=[DataRequired("Це поле має відповідати полю паролю."),
                                                 EqualTo('password')])
...
class UpdateAccountForm(FlaskForm):
    ...
    def validate_email(self, email):
        if email.data != current_user.email:
            user = User.query.filter_by(email=email.data).first()
            if user:
                flash('Ця електронна пошта вже зареєстрована. Будь ласка, використовуйте іншу адресу електронної пошти',
                      'danger')
